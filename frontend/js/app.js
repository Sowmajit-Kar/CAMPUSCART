const { useState, useEffect, useMemo, useRef } = React;
    ...
    function App() {
      ...
      const [cart, setCart] = useState([
        { id: 1, title: "Lab Notebook", price: 12.99, qty: 1, image: window.CAMPUS_DATA.products[0].image }
      ]);
      ...
      const handleLogin = (customEmail) => {
        ...
        showToast(`🎉 Authenticated as ${roll}! Welcome to the CampusCart Main Portal.`);
      };
      ...
      const navigateTo = (route) => {
        if (!isLoggedIn && route !== 'overview') {
          setIsLoginOpen(true);
          showToast("🔒 Please sign in with your college ID (@campus.edu) to access " + route.toUpperCase());
          return;
        }
      ...
      const addToCart = (product) => {
        setCart(prev => {
          ...
          return [...prev, { id: product.id, title: product.title, price: product.price, qty: 1, image: product.image }];
        });
      ...
      const startChat = (seller, item) => {
        setActiveChat({
          ...
          item,
          messages: [
            { sender: 'seller', text: `Hey! Thanks for inquiring about ${item.title}. Can you meet at the campus safe desk today?` }
          ]
        });
      };
        ...
        <div class="relative min-h-screen flex flex-col bg-[#fcfcfd]">
          ...
          <div class="fixed top-6 inset-x-0 z-50 flex justify-center px-4 pointer-events-none">
            <header class="floating-pill text-white px-5 py-2.5 rounded-full shadow-2xl flex items-center justify-between gap-4 sm:gap-6 pointer-events-auto max-w-2xl lg:max-w-4xl w-full whitespace-nowrap">
              ...
              >
                ...
                <span class="font-display font-bold text-sm tracking-wider uppercase text-white">CampusCart</span>
              </button>
              ...
              {isLoggedIn ? (
                ...
                <div class="hidden sm:flex items-center gap-2 bg-white/10 px-3.5 py-1 rounded-full text-xs font-semibold flex-shrink-0">
                  ...
                  <span class="text-white font-bold tracking-wider">OVERVIEW & TOUR</span>
                  <span class="text-[10px] text-neutral-400 font-mono pl-1 border-l border-white/20">PRE-LOGIN GATEWAY</span>
                </div>
              )}
                ...
                {isLoggedIn ? (
                  <div class="flex items-center gap-2">
                    <div class="hidden md:flex items-center gap-1.5 bg-white/10 px-2.5 py-1 rounded-full text-xs text-neutral-300 border border-white/10">
                      ...
                      <span class="font-mono text-[11px] text-white font-bold">{currentUser?.roll || '2024CS1089'}</span>
                    </div>
                    <button 
          ...
          {navMenuOpen && (
            <div class="fixed inset-0 z-40 bg-neutral-950/80 backdrop-blur-md flex flex-col justify-center items-center gap-6 text-white text-2xl font-display font-bold animate-in fade-in duration-200">
              <button onClick={() => navigateTo('overview')} class="hover:text-neutral-400">OVERVIEW / ABOUT</button>
              {isLoggedIn ? (
                <>
                  <button onClick={() => navigateTo('home')} class="hover:text-neutral-400">HOME</button>
                  <button onClick={() => navigateTo('marketplace')} class="hover:text-neutral-400">STUDENT MARKETPLACE</button>
                  <button onClick={() => navigateTo('services')} class="hover:text-neutral-400">PEER SKILLS & GIGS</button>
                  <button onClick={() => navigateTo('course')} class="hover:text-neutral-400">ACADEMIC COURSES</button>
                  <button onClick={() => navigateTo('cart')} class="hover:text-neutral-400">MY CART ({cart.length})</button>
                  <button onClick={() => { handleLogout(); setNavMenuOpen(false); }} class="mt-4 px-6 py-2 bg-red-500/20 text-red-300 border border-red-500/30 rounded-full text-base">SIGN OUT</button>
                </>
              ) : (
                <button onClick={() => { setIsLoginOpen(true); setNavMenuOpen(false); }} class="mt-4 px-6 py-2 bg-white text-black rounded-full text-base">STUDENT SIGN IN</button>
              )}
              <button onClick={() => setNavMenuOpen(false)} class="text-sm font-sans text-neutral-400 mt-6 tracking-widest uppercase">✕ Close Menu</button>
            </div>
          )}
          ...
          <main class="flex-1">
            ...
            {currentRoute === 'services' && (
              <ServicesSection onBook={(title) => showToast(`Requested session for "${title}"! Check your college email for details.`)} />
            )}

            {currentRoute === 'course' && (
              <CourseSection onEnroll={() => showToast('Enrolled in Academic Course! TA has been alerted.')} />
            )}
            ...
            {currentRoute === 'cart' && (
              <CartFullView 
                ...
                onCheckout={() => {
                  ...
                  setCart([]);
                  showToast('🎉 Order Placed! In-Campus QR Pickup Token has been created.');
                }}
              />
            )}
          </main>
          ...
          <footer class="bg-neutral-950 text-white relative overflow-hidden pt-20 pb-12 px-6 sm:px-12 border-t border-neutral-900">
            ...
            <div class="max-w-7xl mx-auto relative z-10 grid grid-cols-1 md:grid-cols-4 gap-12 pb-16 border-b border-neutral-800">
              <div class="md:col-span-2 space-y-4">
                ...
                <p class="text-sm text-neutral-400 max-w-sm leading-relaxed">
                  The peer-to-peer ecosystem designed exclusively for college students. Trade textbooks, lab instruments, electronics, and skills with trusted verification.
                </p>
                <div class="flex items-center gap-3 pt-2 text-xs text-neutral-400">
                  <span class="px-3 py-1 rounded-full border border-neutral-800 bg-neutral-900">@campus.edu only</span>
                  <span class="px-3 py-1 rounded-full border border-neutral-800 bg-neutral-900">CCTV Safe Handovers</span>
                </div>
              </div>

              <div>
                <h4 class="text-xs uppercase font-bold tracking-wider text-neutral-400 mb-4">Marketplace</h4>
                <ul class="space-y-2 text-sm text-neutral-300">
                  <li><button onClick={() => navigateTo('marketplace')} class="hover:text-white">Engineering Drafters</button></li>
                  <li><button onClick={() => navigateTo('marketplace')} class="hover:text-white">Casio Calculators</button></li>
                  <li><button onClick={() => navigateTo('marketplace')} class="hover:text-white">Lab Manuals & Notes</button></li>
                  <li><button onClick={() => navigateTo('marketplace')} class="hover:text-white">Hostel Living Essentials</button></li>
                  <li><button onClick={() => navigateTo('marketplace')} class="hover:text-white">Campus Cycles</button></li>
                </ul>
              </div>

              <div>
                <h4 class="text-xs uppercase font-bold tracking-wider text-neutral-400 mb-4">Verification & Safety</h4>
                <ul class="space-y-2 text-sm text-neutral-300">
                  <li><a href="#" class="hover:text-white">Library Safe Desk</a></li>
                  <li><a href="#" class="hover:text-white">Canteen Meetup Zone</a></li>
                  <li><a href="#" class="hover:text-white">Hostel Entrance Desks</a></li>
                  <li><a href="#" class="hover:text-white">Graph Trust Score</a></li>
                  <li><a href="#" class="hover:text-white">QR Handshake Protocol</a></li>
                </ul>
              </div>
            </div>
            ...
            <div class="max-w-7xl mx-auto pt-8 flex flex-col sm:flex-row items-center justify-between text-xs text-neutral-500 relative z-10 gap-4">
              ...
              <div class="flex gap-6">
                ...
                <a href="#" class="hover:text-neutral-300">Student Code of Conduct</a>
                <a href="#" class="hover:text-neutral-300">Campus Security Helpline</a>
              </div>
            </div>
          </footer>
          ...
          {isLoginOpen && (
            <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
              <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl border border-neutral-100 text-center relative animate-in fade-in zoom-in duration-200">
                ...
                <p class="text-xs text-neutral-500 mb-5">Access restricted to verified university domain accounts.</p>
                ...
                <div class="relative flex py-2 items-center mb-3">
                  ...
                  <span class="flex-shrink mx-3 text-[11px] text-neutral-400 font-mono uppercase">Or credentials</span>
                  <div class="flex-grow border-t border-neutral-200"></div>
                </div>
                ...
                <form onSubmit={(e) => {
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-neutral-600 uppercase tracking-wider mb-1.5">College Email Address</label>
                    <input 
                      type="email" 
                      required 
                      value={loginEmail}
                  ...
                  <div>
                    <label class="block text-xs font-semibold text-neutral-600 uppercase tracking-wider mb-1.5">Password</label>
                    <input 
                      type="password" 
                      required 
                      value={loginPassword}
          ...
          {selectedSeller && (
            <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
              <div class="bg-white rounded-3xl max-w-md w-full p-8 shadow-2xl relative">
                <button onClick={() => setSelectedSeller(null)} class="absolute top-6 right-6 text-neutral-400 text-2xl leading-none">&times;</button>
                <div class="flex items-center gap-4 mb-6">
                  <img src={selectedSeller.avatar} class="w-16 h-16 rounded-2xl object-cover border-2 border-neutral-900" alt="" />
                  <div>
                    <h4 class="font-display text-xl font-bold text-neutral-900">{selectedSeller.name}</h4>
                    <p class="text-xs text-neutral-500">{selectedSeller.department}</p>
                ...
                <div class="p-4 rounded-2xl bg-neutral-50 border border-neutral-200 mb-4 space-y-2 text-xs">
                  <div class="flex justify-between items-center">
                    <span class="font-semibold text-neutral-700">Mutual Graph Trust Score</span>
                    <span class="font-black text-emerald-600 text-sm">{selectedSeller.trustScore} / 100</span>
                  </div>
                  <div class="w-full bg-neutral-200 h-2 rounded-full overflow-hidden">
                    <div class="bg-emerald-600 h-full rounded-full" style={{ width: `${selectedSeller.trustScore}%` }}></div>
                  </div>
                  <p class="text-[11px] text-neutral-400">Calculated from 0 report flags, 28 verified handovers, and in-person ratings.</p>
                </div>
            ...
            <div class="fixed inset-y-0 right-0 z-50 w-full sm:w-96 bg-white shadow-2xl border-l border-neutral-200 flex flex-col">
              <div class="p-4 border-b border-neutral-200 flex items-center justify-between bg-neutral-50">
                <div class="flex items-center gap-3">
                  <img src={activeChat.seller.avatar} class="w-9 h-9 rounded-full object-cover" alt="" />
                  <div>
                    <h4 class="font-bold text-sm text-neutral-900">{activeChat.seller.name}</h4>
                    <p class="text-[10px] text-neutral-500 truncate max-w-[180px]">Re: {activeChat.item.title}</p>
                  </div>
                </div>
                <button onClick={() => setActiveChat(null)} class="text-neutral-400 text-2xl">&times;</button>
              </div>

              <div class="flex-1 p-4 overflow-y-auto space-y-3 text-xs custom-scroll bg-neutral-100/50">
                <div class="text-center text-[10px] text-neutral-400 py-1">🔒 In-Campus Verified Peer Chat</div>
                {activeChat.messages.map((m, idx) => (
                  <div key={idx} class={`flex ${m.sender === 'buyer' ? 'justify-end' : 'justify-start'}`}>
                    <div class={`p-3 rounded-2xl max-w-[80%] leading-relaxed ${m.sender === 'buyer' ? 'bg-neutral-950 text-white rounded-br-xs' : 'bg-white border border-neutral-200 text-neutral-800 rounded-bl-xs'}`}>
                      {m.text}
                    </div>
                  </div>
                ))}
                ...
                onSubmit={(e) => {
                  ...
                  setTimeout(() => {
                    setActiveChat(prev => prev ? ({
                      ...prev,
                      messages: [...prev.messages, { sender: 'seller', text: "Deal! I will bring it to the Library Safe Desk at 4 PM." }]
                    }) : null);
                  }, 1100);
                }} 
              ...
              >
                ...
                />
                <button type="submit" class="px-4 py-2 bg-neutral-950 text-white font-bold rounded-xl text-xs hover:bg-neutral-800">
                  Send
                </button>
              </form>
            </div>
          ...
          {qrModalItem && (
            <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
              <div class="bg-white rounded-3xl max-w-sm w-full p-6 text-center space-y-4 shadow-2xl relative">
                <button onClick={() => setQrModalItem(null)} class="absolute top-4 right-4 text-neutral-400 text-2xl leading-none">&times;</button>
                <span class="text-[10px] uppercase font-bold tracking-widest bg-emerald-100 text-emerald-800 px-2.5 py-0.5 rounded-full">
                  Safe Pickup Token
                </span>
                <h3 class="font-display text-xl font-bold text-neutral-900">{qrModalItem.title}</h3>
                <p class="text-xs text-neutral-500">
                  Show this QR code to <strong>{qrModalItem.seller.name}</strong> at <em>{qrModalItem.pickupLocation}</em>.
                </p>
                ...
                <button 
                  onClick={() => {
                    ...
                    setQrModalItem(null);
                    showToast('🎉 QR Handover Verified! Both buyer & seller trust score increased.');
                  }} 
